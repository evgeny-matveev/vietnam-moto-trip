<script>
	...
	let { stay, compact = false } = $props();
</script>

{#if stay.photos?.length}
	{#if compact}
		<div class="grid grid-cols-2 gap-1 sm:grid-cols-4" aria-label={`Фотографии: ${stay.name}`}>
			{#each stay.photos as photo, index}
				<img
					src={photo}
					alt={`${stay.name}: фото ${index + 1}`}
					loading="lazy"
					decoding="async"
					class="aspect-[4/3] min-w-0 w-full rounded object-cover"
				/>
			{/each}
		</div>
	{:else}
